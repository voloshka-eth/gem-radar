...
export function resolveRpcEndpoints(
  ...
  // Solana: single Connection URL. Prefer Infura when configured for solana.
  const solExplicit = trimOrUndefined(env.solanaRpcUrl);
  const solanaInfura = infuraKey && onInfura('solana')
    ? infuraHttpUrl('solana-mainnet', infuraKey)
    : undefined;
  const solanaAlchemy = alchemyKey && onAlchemy('solana')
    ? alchemyHttpUrl('solana-mainnet', alchemyKey)
    : undefined;
  let solanaFinal: string = PUBLIC_DEFAULTS.solana;
  if (solExplicit) {
    solanaFinal = solExplicit;
  } else if (paidFirst) {
    solanaFinal = solanaInfura ?? solanaAlchemy ?? PUBLIC_DEFAULTS.solana;
  } else {
    // free_first but Solana has no multi-URL Connection: use Infura/Alchemy
    // when assigned so the lane is usable; public remains the default if no key.
    solanaFinal = solanaInfura ?? solanaAlchemy ?? PUBLIC_DEFAULTS.solana;
  }
